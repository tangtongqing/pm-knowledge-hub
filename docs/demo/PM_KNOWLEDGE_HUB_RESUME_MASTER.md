...
### 17.1 目标岗位 JD 原文 (ToB/实施型 PM / 需求分析师)
...
### 17.2 结合本项目的定制面试高频题与高分回答

> **JD 核心特征解读**：这是一个偏向 **ToB / 系统实施 / 需求分析师 (SA)** 的岗位。面试官会非常看重：**数据库建模与 SQL 能力**、**标准化设计文档编制规范（SRS/PRD/概要/详细设计）**、**原型交互设计** 以及 **项目全流程跟进与测试确认能力**。

#### Q1：看你岗位资格要求熟悉数据库，你在 PM Knowledge Hub 项目中是如何进行数据库模型设计的？
*   **【面试官意图】**：考察数据库建模思维（PowerDesigner/MySQL）、数据结构设计能力及技术理解力。
*   **【高分回答】**：
    *   **架构设计**：在本项目中，我采用了“结构化业务数据 + 非结构化向量数据”的混合存储模型。
    *   **核心表结构设计**：
        1. **文档元数据表 (Document Metadata)**：记录 `doc_id`、文件名、字符数、解析状态（`status: pending/processing/completed/failed`）、创建时间等；
        2. **切片向量映射表 (Vector Store Chunk)**：在向量数据库 (ChromaDB) 中设计切片结构，存储 `chunk_id`、`doc_id` 关联外键、Embedding 向量值、字符偏移量区间（`start_offset`, `end_offset`）；
        3. **对话与评估记录表 (Interview Session & Evaluation)**：记录 `session_id`、用户回答、STAR 维度的评分与意见。
    *   **工具与规范**：我熟练使用 **PowerDesigner** 进行概念模型（CDM）与物理模型（PDM）绘制，遵循数据库第三范式（3NF）；熟练编写 SQL 脚本进行多表关联查询（`JOIN`）、分组聚合（`GROUP BY`）及索引优化。

---

#### Q2：JD 要求编写“需求规格说明书、概要设计、详细设计文档”，你的项目中是如何规范输出这些文档的？
*   **【面试官意图】**：考察文档撰写规范、逻辑严密程度与工程标准化素养。
*   **【高分回答】**：
    *   本项目严格遵循软件工程标准化文档体系，输出了完整的三层文档矩阵：
        1. **需求规格说明书 (PRD/SRS)**：定义业务背景、用户角色、用例（Use Cases）、功能需求与非功能需求（性能阈值、并发量、响应时间）。
        2. **概要设计文档 (High-Level Design)**：定义系统总体架构图（前端 ➔ API ➔ RAG 检索引擎 ➔ 模型层）、数据流转图、模块划分与组件关系。
        3. **详细设计文档 (Detailed Design)**：详述 API 接口定义（包含 OpenAPI/FastAPI 的 Request/Response Schema）、数据库字段类型与字典表、状态机流转逻辑（如模拟面试三轮状态转换）及异常降级处理分支。

---

#### Q3：你如何与客户/用户沟通需求，并将其转化为系统原型与业务流程图？
*   **【面试官意图】**：考察 JD 第 1、2 条（需求调研、流程梳理、Axure 原型绘制）。
*   **【高分回答】**：
    *   **需求调研**：先编制《需求调研提纲》，避免漫无目的闲聊。采用“场景化提问”挖掘深层痛点，而非简单照收客户提出的“功能点”。
    *   **流程梳理**：先梳理主干业务流程图（Swimlane / Flowchart），标明角色、输入输出与分支条件，再补充异常流程（如断网、格式不支持、接口超时）。
    *   **原型设计**：使用 **Axure** 绘制中/高保真原型，建立模块化组件库。在原型旁边附带详细的**交互说明文本**（包含：字段校验规则、空状态展示、Loading 动效、错误提示文案）。

---

#### Q4：在开发过程中，你如何与开发人员沟通设计细节、跟进进度并做功能测试确认？
*   **【面试官意图】**：考察 JD 第 5 条（开发沟通、进度跟进、测试确认与质量把控）。
*   **【高分回答】**：
    *   **沟通与跟进**：制定明确的 **DoR（准入标准）** 确保需求评审无死角；通过 Scrum 敏捷看板和 TASK 编号跟踪每日进度，发现阻塞（Blocker）第一时间协调解决。
    *   **测试确认 (DoD 门禁)**：
        1. **测试用例编写**：根据 PRD 编写验收测试用例（覆盖正常流、边界值、异常流）；
        2. **自动化与人工走查**：后端通过 `pytest` 跑通数据与算法接口测试，前端进行多终端与交互走查；
        3. **UAT 验收确认**：对照验收标准（Acceptance Criteria）逐项 Check，确保无 Blocking/P0 级别 Bug 后方可签字确认交付。

---

#### Q5：系统上线后，你如何开展用户功能培训与日常运维工作？
*   **【面试官意图】**：考察 JD 第 6、7 条（用户培训、运维与系统持续维护）。
*   **【高分回答】**：
    *   **用户培训**：编制《用户操作手册（User Manual）》与 FAQ 常见问题解答，针对管理员与普通用户录制操作示范视频，开展分角色培训。
    *   **日常运维**：建立“日志排查与问题闭环机制”。当收到报错时，通过查看系统日志（Logging）、数据库状态及 API 响应码，快速定位是网络波动、数据格式错误还是模型服务异常，推动开发修复并更新知识库。
