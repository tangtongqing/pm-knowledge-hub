...
> **面试建议**：这道题是高频核心题。请根据你面试岗位的侧重点，从以下 4 个故事中选择 **最契合的 1 个** 进行回答（采用标准的 **STAR 法则** 结构）。

##### 选项 1：应对大模型的不稳定性（适合偏 技术 / 工程 / 架构 的 PM 岗位）
* **Situation (情境)**：在接入外部大模型 API 时，由于网络延迟、供应商限制或服务波动，AI 响应经常超时（超过 10 秒）甚至直接报错，导致前端页面长时间白屏，用户体验极差。
* **Task (任务)**：保障系统高可用性（Availability），不能因第三方 API 故障导致核心演示与使用流程瘫痪。
* **Action (行动)**：设计“分级降级机制”。设置 5 秒超时阈值，超时或报错时不无限重试，而是立即触发 Mock 降级（返回本地预设高质量静态数据）；同时在前端增加 Skeleton 骨架屏与提示，并在数据中标注“服务降级”标识。
* **Result (结果)**：即使在第三方 API 完全不可用的情况下，系统核心流程仍能 100% 正常演示，验证了系统的容错与降级设计能力。

##### 选项 2：解决 RAG “不知出处”的信任危机（适合偏 业务 / AI 应用落地 岗位）
* **Situation (情境)**：早期的 RAG 检索回答虽然有内容，但用户无法核实答案真实性，担心 AI 产生“幻觉（Hallucination）”，导致对系统信任度低。
* **Task (任务)**：建立完整的证据闭环，让 AI 生成的每一句回答都有迹可循、可被验证。
* **Action (行动)**：重构后端 Prompt 与数据返回结构，要求必须输出带 Sources 索引的响应；前端设计类似学术论文的引用编号（如 `[1]`、`[2]`），结合悬浮卡片（Hover 预览原文）与点击直接高亮跳转至源笔记锚点（Anchor）的交互。
* **Result (结果)**：实现了“回答 - 摘录 - 原始笔记”的完整证据闭环，使用户置信度显著提升，产品从“对话玩具”升级为“严谨的知识检索工具”。

##### 3. 复杂数据可视化带来的前端性能瓶颈（适合偏 体验 / 交互 / 客户端 岗位）
* **Situation (情境)**：当知识库导入超过 200+ 篇笔记且图谱节点繁多时，前端页面卡顿明显，DOM 渲染严重阻塞主线程。
* **Task (任务)**：在保留大容量数据可视化展示的前提下，优化列表与图谱渲染性能，保持 60fps 流畅交互。
* **Action (行动)**：1. 长列表采用**虚拟滚动（Virtualization）**，只渲染可视区域节点；2. 使用 React `Memo` 避免无意义重渲染；3. 知识图谱引入**动态聚合降级**，超大节点量时先展示 Cluster 聚类，点击后再展开，并优化力导向图（Force-directed graph）物理仿真收敛算法。
* **Result (结果)**：列表卡顿感完全消除，大图谱加载与渲染时间缩短 70% 以上，保证了海量数据下的流畅体验。

##### 4. 范围蔓延导致“文档承诺超前于代码”（适合偏 项目管理 / 交付执行 岗位）
* **Situation (情境)**：开发中期想法较多，PRD 持续规划了多模态、重排等高级功能，但个人开发精力有限，导致文档承诺远超代码进度，交付面临延期风险。
* **Task (任务)**：控制范围蔓延（Scope Creep），明确 MVP 交付边界，确保按期上线可运行版本。
* **Action (行动)**：进行需求冻结与裁切。确立 v1.6 为硬性交付基线（Baseline），将高级图谱过滤、流式输出等非核心功能强行划入 v1.7 边界表（Backlog），主攻“导入-检索-问答-评估”核心闭环。
* **Result (结果)**：按时交付了包含核心闭环的可运行版本，体现了敏捷开发中“Done is better than perfect”的范围管理能力。
